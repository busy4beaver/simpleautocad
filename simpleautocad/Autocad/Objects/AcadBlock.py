...
class IAcadBlock(IAcadObjectCollection):
    ...
    def AddArc(
        self, Center: PyGePoint3d, Radius: float, StartAngle: float, EndAngle: float
    ) -> AcadArc:
    ...
    def AddBox(
        self, Origin: PyGePoint3d, Length: float, Width: float, Height: float
    ) -> Acad3DSolid:
    ...
    def AddCone(
        self, Center: PyGePoint3d, BaseRadius: float, Height: float
    ) -> Acad3DSolid:
    ...
    def AddCylinder(
        self, Center: PyGePoint3d, Radius: float, Height: float
    ) -> Acad3DSolid:
    ...
    def AddDim3PointAngular(
        ...
        TextPoint: PyGePoint3d,
    ) -> AcadDim3PointAngular:
        return AcadDim3PointAngular(
            self._obj.AddDim3PointAngular(
    ...
    def AddDimAligned(
        ...
        return AcadDimAligned(
            self._obj.AddDimAligned(ExtLine1Point(), ExtLine2Point(), TextPosition())
        )

    def AddDimArc(
        self,
        ArcCenter: PyGePoint3d,
        FirstEndPoint: PyGePoint3d,
        SecondEndPoint: PyGePoint3d,
    ...
    def AddDimDiametric(
        self, ChordPoint: PyGePoint3d, FarChordPoint: PyGePoint3d, LeaderLength: float
    ) -> AcadDimDiametric:
        return AcadDimDiametric(
            self._obj.AddDimDiametric(ChordPoint(), FarChordPoint(), LeaderLength)
        )
    ...
    def AddDimRadial(
        self, Center: PyGePoint3d, ChordPoint: PyGePoint3d, LeaderLength: float
    ) -> AcadDimRadial:
        return AcadDimRadial(
            self._obj.AddDimRadial(Center(), ChordPoint(), LeaderLength)
        )
    ...
    def AddDimRadialLarge(
        ...
        return AcadDimRadialLarge(
            self._obj.AddDimRadialLarge(
                Center(), ChordPoint(), OverrideCenter(), JogPoint(), JogAngle
            )
        )

    def AddDimRotated(
        self,
        XLine1Point: PyGePoint3d,
        XLine2Point: PyGePoint3d,
        DimLineLocation: PyGePoint3d,
        RotationAngle: float,
    ) -> AcadDimRotated:
        return AcadDimRotated(
            self._obj.AddDimRotated(
                XLine1Point(), XLine2Point(), DimLineLocation(), RotationAngle
            )
        )

    def AddEllipse(
        self, Center: PyGePoint3d, MajorAxis: PyGeVector3d, RadiusRatio: float
    ) -> AcadEllipse:
    ...
    def AddEllipticalCone(
        ...
        return Acad3DSolid(
            self._obj.AddEllipticalCone(Center(), MajorRadius, MinorRadius, Height)
        )
    ...
    def AddExtrudedSolid(
        self, Profile: AcadRegion, Height: float, TaperAngle: float
    ) -> Acad3DSolid:
        return Acad3DSolid(self._obj.AddExtrudedSolid(Profile(), Height, TaperAngle))

    def AddExtrudedSolidAlongPath(self, Profile: AcadRegion, Path) -> Acad3DSolid:
    ...
    def AddMInsertBlock(
        ...
        RowSpacing: float,
        ColumnSpacing: float,
        Password: Variant = vObjectEmpty,
    ) -> AcadMInsertBlock:
        return AcadMInsertBlock(
            self._obj.AddMInsertBlock(
                InsertionPoint(),
                Name,
                XScale,
                YScale,
    ...
    def AddMText(
        self, InsertionPoint: PyGePoint3d, Width: float, Text: str
    ) -> AcadMtext:
    ...
    def AddPolyfaceMesh(
        self, VerticesList: PyGePoint3dArray, FaceList: vShortArray
    ) -> AcadPolyfaceMesh:
        return AcadPolyfaceMesh(
            self._obj.AddPolyfaceMesh(VerticesList(), FaceList())
        )
    ...
    def AddRegion(self, ObjectList: vObjectArray) -> vObjectArray:
        return vObjectArray(
            [AcadRegion(region) for region in self._obj.AddRegion(ObjectList())]
        )
    ...
    def AddRevolvedSolid(
        ...
        return Acad3DSolid(
            self._obj.AddRevolvedSolid(Profile, AxisPoint(), AxisDir(), Angle)
        )
    ...
    def AddShape(
        ...
        return AcadShape(
            self._obj.AddShape(Name, InsertionPoint(), ScaleFactor, Rotation)
        )
    ...
    def AddSolid(
        ...
        return AcadSolid(
            self._obj.AddSolid(Point1(), Point2(), Point3(), Point4())
        )
    ...
    def AddSpline(
        self,
        PointsArray: PyGePoint3dArray,
        StartTangent: PyGeVector3d,
        EndTangent: PyGeVector3d,
    ) -> AcadSpline:
        return AcadSpline(
            self._obj.AddSpline(PointsArray(), StartTangent(), EndTangent())
        )
    ...
    def AddText(
        self, TextString: str, InsertionPoint: PyGePoint3d, Height: float
    ) -> AcadText:
        return AcadText(self._obj.AddText(TextString, InsertionPoint(), Height))

    def AddTolerance(
        self, Text: str, InsertionPoint: PyGePoint3d, Direction: PyGeVector3d
    ) -> AcadTolerance:
        return AcadTolerance(
            self._obj.AddTolerance(Text, InsertionPoint(), Direction())
        )

    def AddTorus(
        self, Center: PyGePoint3d, TorusRadius: float, TubeRadius: float
    ) -> Acad3DSolid:
    ...
    def AddWedge(
        self, Center: PyGePoint3d, Length: float, Width: float, Height: float
    ) -> Acad3DSolid:
    ...
    def AttachExternalReference(
        ...
        return AcadExternalReference(
            self._obj.AttachExternalReference(
                PathName,
                Name,
                InsertionPoint(),
                XScale,
...
class AcadBlock(IAcadBlock):
    ...
    BlockScaling = proxy_property('AcBlockScaling', 'BlockScaling', AccessMode.ReadWrite)
    Comments = proxy_property(str, 'Comments', AccessMode.ReadWrite)
    Explodable = proxy_property(bool, 'Explodable', AccessMode.ReadWrite)
    IsDynamicBlock = proxy_property(bool, 'IsDynamicBlock', AccessMode.ReadOnly)
    IsLayout = proxy_property(bool, 'IsLayout', AccessMode.ReadOnly)
    IsXRef = proxy_property(bool, 'IsXRef', AccessMode.ReadOnly)
    ...
    XRefDatabase = proxy_property('AcadDatabase', 'XRefDatabase', AccessMode.ReadOnly)
